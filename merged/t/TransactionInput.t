...
foreach my $in (@inputs){
	$j++;
	warn "Data:".$in->{'data'}."\n";
	my $t_in = CBitcoin::TransactionInput->new({'data' => $in->{'data'}});
	
	warn "($j) Hash:[".$t_in->prevOutHash."|".$in->{'hash'}."] and Index:[".$t_in->prevOutIndex."|".$in->{'index'}."]\n";
	ok ( $t_in->prevOutHash eq $in->{'hash'} , 'Testing Hash: '.$j) || print "Bail out!\n";
	ok ( $t_in->prevOutIndex eq $in->{'index'} , 'Testing Index: '.$j) || print "Bail out!\n";
}
